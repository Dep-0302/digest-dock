...
2026-08-24 用户验收修正：笔记 Markdown 的内容范围是标题、频道、网址、视频简介和已保存笔记，不附带整部视频字幕；完整字幕只属于字幕 TXT。旧文中“笔记导出包含完整字幕”的表述由本修正覆盖。

## 1. 问题与根因

当前产品分别保存单条笔记中文、视频标题中文、页面渐进字幕中文和导出来源资料。旧实现把笔记 Markdown 当成“整部视频阅读包”，导致一条笔记也附带数百条完整字幕，并要求先补齐整部字幕才能导出。修正后笔记和字幕使用同一持久资料层，但规划与文件内容严格分域：笔记只补标题、简介和已保存笔记；字幕 TXT 才补完整字幕段。
...
笔记预检只显示标题、简介块和已保存笔记的完成／缺失数量；字幕预检只显示完整字幕段。两个范围不得互相阻断。

示例文案：

> 3 条笔记均已翻译。还需补齐 1 个简介块。本轮完成后自动导出；每批完成后立即保存。

长字幕 TXT 示例：

> 已完成 146 / 395 段字幕，还需补齐 249 段，共约 63 批。本轮最多执行 20 批；每批完成后立即保存，结束后原位显示“继续补齐”。
...
- 字幕页与字幕下载共用完整字幕翻译；笔记导出只复用标题、简介和笔记译文，不规划完整字幕。
...
- 已翻译笔记不重新调用 Provider。
- 字幕页翻译继续供字幕 TXT 复用，但不再阻断笔记 Markdown。
- 笔记导出的标题、简介和笔记译文可跨三种笔记导出入口复用。
- 简介与字幕齐全后中文/双语自动下载。
- 原文导出始终零 Provider 请求。
...
1. 长视频不再因总量上限永久禁用。
2. 已完成内容不会重复翻译，且笔记与字幕只规划各自文件需要的内容。
...
- 字幕导出只补字幕；笔记 Markdown 只补标题、简介和已保存笔记，不再包含或补译整部字幕。
