...
(function installYouTubeActive(root, factory) {
  ...
  const PROVIDER_VARIANT = "isolated-tab";
  const PLAYER_ENDPOINT =
    "https://www.youtube.com/youtubei/v1/player?prettyPrint=false";
  const TIMEOUT_MS = 15_000;
  const MAX_RESPONSE_BYTES = 8 * 1024 * 1024;
  const MAX_PLAYER_REQUESTS = 4;
  const MAX_TIMEDTEXT_REQUESTS = 3;
  ...
  async function fetchBoundedText(
    ...
    const timeoutId = runtime.setTimeout(() => controller.abort(), TIMEOUT_MS);
    ...
    try {
      ...
      runtime.runState.controllers.delete(controller);
    }
  }
  ...
  async function run(request = {}, deps = {}) {
    ...
    try {
      ...
      for (const profile of runtime.clients) {
        ...
        // to another client/track after its three bounded formats are exhausted.
