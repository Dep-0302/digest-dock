...
```
You're my executive assistant. I'm interested in this YouTube video. Read the source-language transcript attached and produce a concise Simplified Chinese structural overview with chapters and key quotes. Generate the Chinese overview directly from the transcript; do not draft an English overview first.
...
- 3-5 key quotes from the transcript with their timestamps. Put the polished quote in the source language in `quoteOriginal`, and its faithful Simplified Chinese rendering in `quoteZh`. If the source language is Chinese, copy the same polished Chinese quote into both fields; do not translate it.
