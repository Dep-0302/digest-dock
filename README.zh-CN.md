...
JSON 是用于恢复 DigestDock 笔记的数据备份格式，与阅读导出相互独立。笔记页提供当前视频、全部笔记和单个视频来源的 UTF-8 Markdown 导出；每个视频包含标题、频道、网址、简介和按时间码排序的已保存笔记，不再附带整部字幕。完整字幕由字幕页按当前原文、中文或双语模式单独下载 UTF-8 TXT。

原文阅读导出只使用本地资料，不调用 AI 或 Supadata。笔记 Markdown 只检查标题、简介和已保存笔记的译文，不再要求补齐整部字幕；字幕 TXT 才单独检查完整字幕段。DigestDock 不会用原文冒充中文，只有你点击“生成中文并导出”或“继续补齐”后才会使用当前 AI 服务商。长字幕可分轮推进，每个有效批次立即保存，继续按钮会保留到文件可导出。关闭侧栏、取消或隔天回来时，原文未变化的已完成译文不会重新翻译。导出补译不会调用 Supadata，也不会静默切换 AI 服务商。
