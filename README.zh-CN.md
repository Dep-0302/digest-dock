...
- 保存带时间戳的金句和自己的想法，跨视频检索，并通过带版本信息的 JSON 备份在设备间迁移。
...
## 3.0.0 新增：记下想法，找回触发它的那一刻

- 按一次 **N** 保存金句，视频继续播放；10 秒提示期间再按 **N**，暂停并输入自己的想法。**Enter** 保存，**Shift+Enter** 换行，**Esc** 取消草稿。保存或取消后，如果进入输入框前正在播放，就恢复播放。
- 再次捕获已存的同一段内容，可以从段落内不同字幕行找回原想法，保留原时间点与正文。存在歧义的历史想法不会被自动合并。
- 全局搜索想法、原句、完整金句、已有有效译文、视频标题和频道；清空后恢复原视图。“全部笔记”可按日期或视频排列，并记住选择。
- 精确搜索无结果时，可主动点击 **“让 AI 找找”**，最多返回 5 条已有笔记。AI 不改写你的想法；写想法和编辑想法不需要 API Key。清空或删除想法时，金句与来源仍保留。
- YouTube 有中文轨时优先提取中文，简体、繁体均可。笔记回跳后自动准备缓存或免费原生字幕，侧栏继续停留在笔记页。

本次升级沿用现有笔记和 JSON 备份格式。在同一解压目录更新文件，重新加载扩展并刷新视频页面即可。更新前可导出恢复备份，无需清空笔记或设置。
...
导入会与设备上已有笔记合并，并自动跳过重复项；如果已有笔记缺少某些已保存内容，导入可以用备份中的内容补全。DigestDock 不设固定笔记条数上限。如果相同笔记 ID 的内容发生冲突，或合并后的恢复备份将超过 32 MiB 容量护栏，整次导入都会被拒绝；新保存也使用同一容量护栏。无效、不受支持、过大或因其他原因失败的导入，都不会改变设备上原有的笔记。
...
原文阅读导出只使用本地资料，不调用 AI 或 Supadata。笔记 TXT 只检查所选视频的元数据、标题、简介和已保存笔记，不要求补齐整部字幕；字幕 TXT 才单独检查完整字幕段。资料不完整时可选“完整导出”或“直接导出”：完整导出在你明确确认后，准备已告知的来源页面并补译所选范围；直接导出下载现有内容并明确标注缺失，不会用原文冒充中文。导出语言由顶部语言选择器决定，关闭或 Esc 即取消；补全不调用 Supadata。长任务继续保持分轮、可取消和可恢复。
...
- 全部笔记可按日期或视频来源分组，日期从新到旧，同日笔记按时间码排序；当前视频仍按时间码排列。可选择一个或多个视频导出 TXT，也可导出当前视频、全部视频或单个来源；字幕按当前语言模式单独导出 TXT。
