...
JSON 是用于恢复 DigestDock 笔记的数据备份格式，与阅读导出相互独立。笔记页另提供当前视频、全部笔记和单个视频来源的 UTF-8 Markdown 导出；每个视频包含标题、频道、网址、简介、完整字幕和按时间码排序的笔记。字幕页可按当前原文、中文或双语模式下载 UTF-8 TXT。

原文阅读导出只使用本地资料，不调用 AI 或 Supadata。中文或双语资料尚不完整时，DigestDock 会列出缺少的标题、简介块、字幕段和笔记，不会用原文冒充中文；只有你点击“生成中文并导出”或“继续补齐”后才会使用当前 AI 服务商。长视频可以分成多轮由用户主动推进：每次点击最多启动 20 个任务批次，每个有效批次完成后立即保存到本地，下一轮绝不会自动开始。关闭侧栏、取消后续批次或隔天回来时，已完成译文仍可复用，原文没有变化的内容不会重新翻译。取消时已经发出的当前请求若仍匹配冻结的视频与原文版本，可以进入本地复用缓存，但不能继续下一批或触发自动下载。导出补译不会调用 Supadata，也不会静默切换 AI 服务商。
...
- 全部笔记按视频来源分组，来源内只按时间码排序；当前视频、全部笔记和单个来源可导出 Markdown，字幕可按当前语言模式导出 TXT。
...
日常浏览中的翻译是延迟按需和渐进式的。已缓存的分段会复用，只有滚动到并请求的字幕行才会发起调用。只有明确点击“生成中文并导出”时，才可能补齐显示范围外的缺失内容。长视频会被拆成每轮最多 20 个任务批次；每个有效批次立即保存，后续轮次只规划原文版本仍匹配的缺失单元，且不会自动续跑。重试、服务商行为和价格变化都可能增加最终成本。
...
删除全部笔记或重置扩展数据时，也会清理视频级阅读导出资料和未完成任务；Chrome 已经下载的 Markdown、TXT 或 JSON 文件不会因此被删除。
