const YTD_OPTIONS = (() => {
  ...
  const COPY = {
    en: {
      ...
      notesBackupHelp:
        "The JSON backup contains saved notes with their stored language versions and timestamps. It never includes API keys, settings, full transcripts, or digest caches. Import merges with local notes and skips duplicates.",
    ...
    "zh-CN": {
      ...
      notesBackup: "笔记备份",
      notesBackupHelp:
        "JSON 备份只包含已保存笔记及其原文、中文和时间戳，不包含 API 密钥、设置、完整字幕或摘要缓存。导入会与本机笔记合并，并自动跳过重复项。",
      exportNotes: "导出笔记备份",
      importNotes: "导入笔记备份",
      localData: "本地数据",
      localDataHelp:
