...
test("cross-platform runtime dependencies match the Passive-first then fixed-Active release surface", () => {
  ...
  assert.match(optionsPage, /<p class="settings-version">DigestDock 3\.0\.1<\/p>/);
...
test("side panel MVP keeps identity and navigation persistent while transcript state stays local", () => {
  ...
  assert.match(html, /id="transcriptSectionTitle"/);
  assert.match(html, /id="transcriptCoverageNotice"/);
