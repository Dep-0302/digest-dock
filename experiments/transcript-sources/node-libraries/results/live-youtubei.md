# Node 无 Key 字幕库真实对比：live-youtubei

运行时间：2026-08-26T06:57:39.272Z；环境：v22.23.1 / darwin-arm64。
...
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| youtubei.js | 0% (0/1) | — (0/0) | — (0/0) | 2810 ms | 4 | {"upstream-api-error":1} |

## 每个真实样本

| 样本 | 预期 | youtubei.js |
| --- | --- | --- |
| many-tracks-authored-en | transcript (en) | 错误 upstream-api-error / 2810 ms |
...
- youtubei.js 在本轮 1/1 次调用中报错：upstream-api-error: Request to https://www.youtube.com/youtubei/v1/get_transcript failed with status code 400。
- HTTP 日志只保存 origin + path，不保存字幕 URL 查询串、API key 参数、Cookie 或请求正文。JSON 不保存完整字幕，只保存段数、字符数与 SHA-256。

## 本组建议边界

仅在“需要 Node 伴随进程”的候选里，本轮应优先继续验证 youtubei.js：它的正样本成功率为 0%，安装足迹约 18.86 MiB。这不是直接采用结论；还应与纯扩展 MAIN-world / transcript-panel 路径比较，因为 Node 库意味着额外运行时边界。
