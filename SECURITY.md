...
- YouTube transcript retrieval is local-first: the extension reads existing page caption tracks and their `timedtext` responses, and may continue with additional non-WEB YouTube player client profiles. The fixed player POST runs through a minimal MAIN-world page-origin bridge; the trusted `timedtext` GET uses the active tab's extension-isolated bridge. Every request must use `credentials: "omit"`, validate the current video identity, and remain bounded by timeout and response-size limits; parsing and provider configuration stay in the extension.
...
- Supadata is an optional failure fallback for recoverable local extraction failures only. Login or age restrictions, unavailable videos, explicit no-caption results, and temporary YouTube rate limits must stop locally. For an eligible failure, Supadata may receive the canonical YouTube URL only after the user has saved a key and explicitly confirms that one attempt in the side panel. Consent must not be persisted or inferred from the saved key. Saving Settings must not require that optional key.
